...
public class Constants {
	...
	static public class DB2Queries {
		...
		public static final String topVisitorsNoHotel = "SELECT C1 AS COUNTRY, ROUND(TOTAL) AS TOTAL, HOTEL, ROUND(((100-HOTEL)/100)* TOTAL) AS UNBOOKED "
				...
				+ "GROUP BY COUNTRY) AS T2 WHERE T1.C1 = T2.C2 " + "ORDER BY UNBOOKED DESC LIMIT 5";
		...
		public static final String hotelOcupancyRoomRate = "SELECT TIER, ROUND(AVG(AOR),2) AS AOR, ROUND(AVG(REVPAR),2) AS REVPAR, ROUND(AVG(ARR),2) AS ARR "
				+ "FROM HOTELS_TIER_MONTHLY WHERE MONTH = %d GROUP BY MONTH,TIER";
